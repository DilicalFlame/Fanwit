...
function updateAppConstants(version) {
    if (fs.existsSync(paths.appConstants)) {
        ...
        fs.writeFileSync(paths.appConstants, content, "utf8");
        updatedFiles.push(path.relative(rootDir, paths.appConstants));
    }
}
